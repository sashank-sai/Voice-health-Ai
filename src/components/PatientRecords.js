...
import { patients, kpiData } from '../data/mockData';
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                <StatCard title="Patients in Treatment" value={kpiData.treatmentActive.toString()} subtext="+14 this week" icon={Users} gradient="bg-gradient-to-br from-violet-400 to-violet-600" />
                <StatCard title="Treatment Completed" value={kpiData.treatmentCompleted.toString()} subtext="+28 this month" icon={Activity} gradient="bg-gradient-to-br from-emerald-400 to-emerald-600" />
                <StatCard title="Active Users" value={kpiData.activeUsers.toLocaleString()} subtext={`+${kpiData.newUsersToday} today`} icon={Users} gradient="bg-gradient-to-br from-blue-400 to-blue-600" />
                <StatCard title="Avg Call Duration" value={kpiData.avgCallDuration} subtext="Per consultation" icon={Activity} gradient="bg-gradient-to-br from-amber-400 to-amber-600" />
            </div>
                ...
                {patients.map((p) => (
                    <div key={p.id} className="bg-white rounded-2xl border border-slate-100 p-6 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                        <div className="flex justify-between items-start mb-4">
                            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-violet-400 to-violet-600 flex items-center justify-center text-white font-bold text-lg shadow-lg">
                                {p.name.charAt(0)}
                            </div>
            ...
            <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
                <div className="p-8 border-b border-slate-100">
                    ...
                    <p className="text-sm text-slate-500 mt-1">Patients currently under treatment or monitoring</p>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50 text-slate-400 text-xs font-bold uppercase tracking-wider">
                                <th className="p-6">ID</th><th className="p-6">Name</th><th className="p-6">Age</th>
                                <th className="p-6">Condition</th><th className="p-6">Hospital</th><th className="p-6">Visits</th><th className="p-6">Status</th>
                            </tr>
                        </thead>
